...
class FuelTypeNormalizer {
  ...
  static FuelType normalizeFuelType(String? raw) {
    ...
    if (value.contains('가솔린') || value.contains('휘발유') || value == 'gasoline' || value == 'gas') {
      return FuelType.gasoline;
    }
    if (value.contains('디젤') || value.contains('경유') || value == 'diesel') {
      return FuelType.diesel;
    }
    if (value.contains('플러그인') || value == 'phev' || value == 'plug_in_hybrid' || value == 'plugin_hybrid') {
      return FuelType.plugInHybrid;
    }
    if (value.contains('하이브리드') || value == 'hybrid' || value == 'hev') {
      return FuelType.hybrid;
    }
    if (value.contains('전기차') || value.contains('전기') || value == 'electric' || value == 'ev') {
      return FuelType.electric;
    }
    if (value.contains('lpg') || value.contains('lpi') || value == 'lpg' || value == 'lpi') {
      return FuelType.lpg;
    }
    if (value.contains('수소차') || value.contains('수소') || value == 'hydrogen' || value == 'fcev') {
  ...
  static PowertrainType normalizePowertrainType(String? raw, FuelType fuelType) {
    ...
    if (value == 'hybrid' || value.contains('하이브리드')) return PowertrainType.hybrid;
    if (value == 'plug_in_hybrid' || value == 'phev' || value.contains('플러그인')) return PowertrainType.plugInHybrid;
    if (value == 'battery_electric' || value == 'electric' || value == 'ev' || value.contains('배터리') || value.contains('전기')) {
      return PowertrainType.batteryElectric;
    }
    if (value == 'fuel_cell' || value == 'fcev' || value.contains('연료전지') || value.contains('수소')) {
